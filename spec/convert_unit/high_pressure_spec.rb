module ConvertUnit
  RSpec.describe HighPressure, type: :class do
    context 'initiate object with wrong unit type argument' do
      it 'raise a TypeError' do
        expect { HighPressure.new(1, 2) }.to raise_error(TypeError, 'Invalid Unit Type')
      end
    end

    context 'initiate object with worng unit argument' do
      it 'raise a TypeError' do
        expect { HighPressure.new(1, 'degre') }.to raise_error(TypeError, 'Invalid Unit Type')
      end
    end

    context 'initiate object with wrong value' do
      it 'raise a TypeError' do
        expect { HighPressure.new('abc', 'cm2') }.to raise_error(TypeError)
      end
    end

    context 'initiate object with valid value' do
      it 'not raise any error' do
        expect { HighPressure.new(1, 'kilopascal') }.to_not raise_error
      end
    end

    context 'initiate object with short form of unit' do
      it 'not raise any error' do
        expect { HighPressure.new(1, 'kpa') }.to_not raise_error
      end
    end
  end
  ...
  describe '.load_units_symbol' do
    it 'return ruby hash of length unit symbol form' do
      expect(HighPressure::UNITS_SYMBOL).to eq('bar' => 'bar', 'pound/square_inch' => 'psi',
  ...
  describe '.load_units' do
    it 'return list of valid units' do
      expect(HighPressure::UNITS).to eq(%w[bar psi kpa mpa kgf/cm2 mm_hg atm])
    end
  end

  describe 'check equal method' do
    let(:l1) { HighPressure.new 3.1415678, 'psi' }
    let(:l2) { HighPressure.new 3.1415678, 'bar' }
  ...
  describe 'check === method' do
    let(:l1) { HighPressure.new 1093.613, 'bar' }
    let(:l2) { HighPressure.new 39370.08, 'atm' }
    let(:l3) { HighPressure.new 1093.613, 'bar' }
  ...
  describe 'check + method' do
    let(:bar) { HighPressure.new 1093.613, 'bar' }
    let(:psi) { HighPressure.new 1000, 'psi' }
